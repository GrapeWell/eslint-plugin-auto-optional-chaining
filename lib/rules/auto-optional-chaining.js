...
module.exports = {
  ...
  create: function (context) {
    ...
    function processChain(node, context) {
      ...
      // 定义常见数组方法列表，这些方法不应添加可选链调用
      const arrayAndStringMethods = new Set([
        // 数组方法
        "map",
        "filter",
        "forEach",
      ...
      const isArrayMethodCall =
        ...
        arrayAndStringMethods.has(node.property.name);
      ...
      context.report({
        ...
        fix(fixer) {
          ...
          if (isMethodCall) {
            ...
            // 只有非数组方法才添加可选链调用
            if (!isArrayMethodCall) {
              // 找到左括号位置，在其前插入可选链
              const leftParenToken = sourceCode.getTokenAfter(
                node.property,
                (token) => token.value === "("
              );
              fixes.push(fixer.insertTextBefore(leftParenToken, "?."));
            }
